...
#if defined(CYDEV_BOOTLOADER_IO_COMP) && (EZI2C_BOOTLOADER_INTERFACE_ENABLED)
...
static uint8 XDATA EZI2C_slReadBuf[EZI2C_BTLDR_SIZEOF_READ_BUFFER];

/* I2C read buffer - The host reads responses from it */
static uint8 XDATA EZI2C_slWriteBuf[EZI2C_BTLDR_SIZEOF_WRITE_BUFFER];
...
*
* Global variables:
*  EZI2C_slWriteBuf - The global variable used to store a received
*                                command.
*  EZI2C_slReadBuf -  The global variable used to store a response.
*  EZI2C_slRdBufIndex - The global variable used to store a current
*                                  index within the slave read buffer.
*
*******************************************************************************/
void EZI2C_CyBtldrCommStart(void) CYSMALL 
{
    /* Read returns 0xFF when buffer is zero. Write transaction is expected. */
    EZI2C_SlaveInitWriteBuf(EZI2C_slWriteBuf, EZI2C_BTLDR_SIZEOF_WRITE_BUFFER);
    EZI2C_SlaveInitReadBuf (EZI2C_slReadBuf, 0u);
...
*
*******************************************************************************/
void EZI2C_CyBtldrCommStop(void) CYSMALL 
{
    EZI2C_Stop();
}
...
*
*******************************************************************************/
void EZI2C_CyBtldrCommReset(void) CYSMALL 
{
...
*
*******************************************************************************/
cystatus EZI2C_CyBtldrCommWrite(const uint8 pData[], uint16 size, uint16 * count, uint8 timeOut) CYSMALL
...
*
* Global variables:
*  EZI2C_slWriteBuf - The global variable used to store a
*                                received command.
*
*******************************************************************************/
cystatus EZI2C_CyBtldrCommRead(uint8 pData[], uint16 size, uint16 * count, uint8 timeOut) CYSMALL
         
{
    ...
    if((NULL != pData) && (size > 0u))
    {
        ...
        while(0u != timeoutMs)  /* Wait for command from host */
        {
            ...
            if(0u != (EZI2C_slStatus & EZI2C_SSTAT_WR_CMPLT))
            {
                /* Define how many bytes host has been written */
                byteCount = EZI2C_slWrBufIndex;
                *count = (uint16) byteCount;

                /* Copy command to pData buffer */
                (void) memcpy((void *) pData, (void *) EZI2C_slWriteBuf,
                              EZI2C_MIN_UNT16(byteCount, size));
...
#endif /* defined(CYDEV_BOOTLOADER_IO_COMP) && (EZI2C_BOOTLOADER_INTERFACE_ENABLED) */
